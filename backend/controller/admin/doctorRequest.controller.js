...
exports.getAll = async (req, res) => {
  try {
    ...
    if (!type) {
      return res
        .status(200)
        .send({ status: false, message: "Type is requires" });
    }
...
exports.acceptRequest = async (req, res) => {
  try {
    if (!req.query.requestId) {
      return res
        .status(200)
        .send({ status: false, message: "Request id is required" });
    }
    ...
    return res
      .status(200)
      .send({ status: true, message: "Doctor added successfully", doctor });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
...
exports.rejectRequest = async (req, res) => {
  try {
    ...
    const request = await Request.findById(req.query.requestId).populate(
      "service"
    );
    if (!request) {
      return res
