...
exports.getAppointMent = async (req, res) => {
  try {
    ...
    const pipeline = [
      { $match: { ...getStatus(status), ...dateFilter } },
      { $sort: { date: -1, time: 1 } },
      {
        $lookup: {
          from: "users",
    ...
    const [appointment, total] = await Promise.all([
      ...
      Appointment.countDocuments({ ...getStatusFilter(status), ...dateFilter }),
    ]);
...
exports.getParticularUser = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "Success", total, data: appointment });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
...
// month appointment state for admin
